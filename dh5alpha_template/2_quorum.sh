...
-p 10 \
            ...
            printf "| %s | %s | %s | %s | %s | %s | %s | %s | %s |\n" \
                "Q${Q}L${L}.${PREFIX}" \
                $( perl -e "printf qq(%.1f), ${SUM_IN} / 4583637;" ) \
                $( perl -e "printf qq(%.1f), ${SUM_OUT} / 4583637;" ) \
                $( perl -e "printf qq(%.2f%%), (1 - ${SUM_OUT} / ${SUM_IN}) * 100;" ) \
                $( cat env.json | jq '.KMER' ) \
                $( perl -MNumber::Format -e "print Number::Format::format_bytes(4583637, base => 1000,);" ) \
                $( perl -MNumber::Format -e "print Number::Format::format_bytes(${EST_G}, base => 1000,);" ) \
                $( perl -e "printf qq(%.2f), ${EST_G} / 4583637" ) \
