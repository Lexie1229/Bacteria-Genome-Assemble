...
for PREFIX in R S T; do
    ...
    ## 以上为输入的参数
        ...
        -o trim.sh
    bash trim.sh
    ## --dedupe，Do the dedupe step 删除重复数据
    ## --cutoff <cutoff>，Min kmer depth cutoff 设置最小深度阈值
    ## --cutk <cutk>，Kmer for cutoff [default: 31] 设置kmer大小
    ## -q,--qual <qual>，Quality threshold [default: 25] 设置质量阈值
    ## -l,--len <len>，Filter reads less or equal to this length [default: 60] 设置reads最小长度
    ## --filter <filter>，Adapter, artifact, or both [default: adapter] 除去接头及artifact
    ## \
    ## -p,--parallel <parallel>，Number of threads [default: 8] 设置线程数
    ## --xmx <xmx>，Set Java memory usage 设置使用的内存大小
    ## \
    ## --prefix <prefix>，Prefix of trimmed reads [default: R] 设置修剪后序列的前缀
    ## -o,--outfile <outfile>，Output filename. [stdout] for screen [default: trim.sh] 指定输出文件
