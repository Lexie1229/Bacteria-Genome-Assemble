...
parallel --no-run-if-empty --linebuffer -k -j 1 "
    ...
    echo >&2 '==> 4_unitigs_bcalm/Q{1}L{2}X{3}P{4}'
    if [ -e 4_unitigs_bcalm/Q{1}L{2}X{3}P{4}/unitigs.fasta ]; then
        echo >&2 '    unitigs.fasta already presents'
        exit;
    fi

    mkdir -p 4_unitigs_bcalm/Q{1}L{2}X{3}P{4}
    cd 4_unitigs_bcalm/Q{1}L{2}X{3}P{4}
